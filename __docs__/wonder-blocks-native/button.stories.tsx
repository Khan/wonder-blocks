...
/**
 * SPIKE (FEI-8331, option D). A React Native `Button` rendered through
 * react-native-web. Its styles are compiled at build time from the *web*
 * `button.module.css` + theme variables by `react-native-css`'s compiler,
 * and resolved at runtime by `react-native-css`'s native runtime (not by
 * the browser: react-native-web only ever sees plain RN style objects).
 *
 * The Storybook theme toolbar drives both the web and native components.
 * Native theming is app-wide (see `NativeThemeProvider`).
 */
export default {
 ...
 * change from 8px to 12px. (Unlike the option A spike, this doesn't animate:
 * `react-native-css` needs Reanimated 4 for `transition`.)
 */
 ...
 * every `kind` × `actionType`, plus disabled. Differences here are what
 * `react-native-css` can't (yet) express. Expect focus rings and primary's
 * hover/press `outline` to be missing on native. The secondary/tertiary
 * inset `box-shadow` rings show here, but need the New Architecture on
 * device.
 */
