...
The menu is rendered using semantic list markup: the menu container is a `<ul>`
element and each item is wrapped in a presentational `<li>` element
(`role="none"`). This ensures the items are exposed as a list to assistive
technologies that rely on the underlying HTML, while the `menu`/`menuitem` ARIA
roles are preserved for the menu button interaction pattern. This matches the
[WAI-ARIA Menu Button links](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/examples/menu-button-links/)
pattern.
