...
The calendar is powered by `react-day-picker`, which is included as a dependency
of this package. Its required CSS is also bundled, so you don't need to install
`react-day-picker` or import any additional styles.
...
- **Keyboard navigation**: Tab to focus, Enter/Space/DownArrow to open and select items, Esc to close
- **Input labels**: Configurable via `LabeledField` or directly on the input using the `inputAriaLabel` prop
