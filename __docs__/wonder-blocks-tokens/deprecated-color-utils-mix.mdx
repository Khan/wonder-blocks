...
import {Meta, Story, ColorItem} from "@storybook/addon-docs/blocks";
...
<Meta title="Packages / Tokens / Utilities / mix" tags={["deprecated", "!manifest"]} />
...
<Banner
    ...
    text={
        <>
            <code>mix()</code> is deprecated! We recommend using the native CSS{" "}
            <code>color-mix()</code> function instead, which is supported in all
            modern browsers. The <code>mix()</code> function will be removed in
            a future release.
        </>
    }
    actions={[
        {
            ...
            node: (
                <a href="https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/color_value/color-mix">
                    View color-mix() documentation on MDN
                </a>
            ),
        },
    ]}
