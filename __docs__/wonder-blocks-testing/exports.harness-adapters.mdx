...
type Config = {
     ...
     * a function `(harnessedComponent) => routes` for full control over where
     * the harnessed component is placed (e.g. as a route's `errorElement`).
     */
    routes:
        | Array<RouteObject>
        | ((harnessedComponent: React.ReactNode) => Array<RouteObject>);
...
-   **Array form** — the harnessed component is mounted as the `element` of the route that matches the initial location. The matched leaf route must not define its own `element`, `Component`, or `lazy`. This is the ergonomic default for "the component _is_ the page":
...
-   **Function form** — `routes: (harnessedComponent) => [...]` gives you full control over placement. This is what you want when the component-under-test is an `errorElement`:

    ```tsx
    const Harnessed = testHarness(RouteErrorBoundary, {
        router: {
            routes: (harnessedComponent) => [
                {
                    path: "/items/:id",
                    loader: () => {
                    ...
                    element: <ItemPage />,
                    errorElement: harnessedComponent, // the component under test
                },
            ],
            initialEntries: ["/items/42"],
        },
