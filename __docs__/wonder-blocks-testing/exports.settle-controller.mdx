...
<Meta
    title="Packages / Testing / Mocking / Exports / SettleController"
/>
...
The `SettleController` is used to control the settling of a signal. This is specifically created to work with the <a href="./?path=/docs/packages-testing-mocking-exports-respondwith--docs">`RespondWith`</a> API. The `signal` property it exposes can be passed to `RespondWith` methods and then the `settle` method can be invoked to settle the signal, causing the related responses to either reject or resolve as appropriate.
