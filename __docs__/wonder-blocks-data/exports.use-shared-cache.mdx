...
<Meta
    title="Packages / Data / Exports / useSharedCache()"
/>
...
The `useSharedCache` hook provides access to a shared in-memory cache. This cache is not part of the cache hydrated by Wonder Blocks Data, so <a href="./?path=/docs/packages-data-exports-sharedcache--docs">`SharedCache.purgeAll()`</a> must be called between server-side render cycles.
...
Each cached item is identified by an id and a scope. The scope is used to group items. Whole scopes can be cleared by specifying the specific scope when calling <a href="./?path=/docs/packages-data-exports-sharedcache--docs">`SharedCache.purgeScope()`</a>.
