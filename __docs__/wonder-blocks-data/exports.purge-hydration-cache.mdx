...
<Meta
    title="Packages / Data / Exports / purgeHydrationCache()"
    parameters={{
        chromatic: {
...
This can be used after <a href="./?path=/docs/packages-data-exports-initializehydrationcache--docs">`initializeHydrationCache`</a> to manipulate the cache prior to hydration wich can be useful during testing (especially to clear the cache so that it can be initialized again).
