...
<Meta
    title="Packages / Data / Exports / ScopedInMemoryCache"
    parameters={{
        chromatic: {
...
This class implements an in-memory cache that can contain different scopes of cached data. This allows for quick removal of entire classes of data as identified by their scopes without having to iterate each cached item to find them. It implements the <a href="./?path=/docs/packages-data-types-scopedcache--docs">`ScopedCache`</a> interface.
