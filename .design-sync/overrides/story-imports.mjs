// forked from design-sync lib/story-imports.mjs — map each Wonder Blocks package's
// DEFAULT export (import Button from "@khanacademy/wonder-blocks-button") to its
// component name on the global. The stock bare-package shim maps a default import
// to the whole namespace object, so every default-export component renders
// "Element type is invalid". See NOTES.md.
const WB_PKG_DEFAULTS = {
  "@khanacademy/wonder-blocks-banner": "Banner",
  "@khanacademy/wonder-blocks-birthday-picker": "BirthdayPicker",
  "@khanacademy/wonder-blocks-button": "Button",
  "@khanacademy/wonder-blocks-clickable": "Clickable",
  "@khanacademy/wonder-blocks-icon-button": "IconButton",
  "@khanacademy/wonder-blocks-link": "Link",
  "@khanacademy/wonder-blocks-pill": "Pill",
  "@khanacademy/wonder-blocks-search-field": "SearchField",
  "@khanacademy/wonder-blocks-switch": "Switch",
  "@khanacademy/wonder-blocks-toolbar": "Toolbar",
  "@khanacademy/wonder-blocks-tooltip": "Tooltip",
};
...
export function storyImportPlugins({ PKG, GLOBAL, extraEntries = [], exported, cfg, pkgDir }) {
  ...
  const dsShim = {
    ...
    setup(b) {
      ...
      b.onResolve({ filter: pkgRx }, (a) => {
        ...
        return shimResult(WB_PKG_DEFAULTS[a.path] ?? null);
      });
  ...
  // [WB fork] esbuild dataurl loader emits data:image/svg+xml,<svg ...> with raw
  // <, spaces, " and # — valid in <img src> but Aphrodite injects it as an
  // UNQUOTED CSS url(...) (PhosphorIcon maskImage), which the browser then drops,
  // rendering every icon as a solid square. Base64 has no CSS-breaking chars.
  const svgBase64 = {
    name: 'ds-svg-base64',
    setup(b) {
      b.onLoad({ filter: /\.svg$/, namespace: 'file' }, (a) => ({
        contents: `export default "data:image/svg+xml;base64,${readFileSync(a.path).toString('base64')}";`,
        loader: 'js',
      }));
    },
  };
  ...
  return {
    plugins: [dsShim, storybookStubPlugin(), consoleStub, svgBase64, policyRedirect],
