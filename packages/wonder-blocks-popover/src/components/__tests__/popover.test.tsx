...
describe("Popover", () => {
    ...
    describe("a11y", () => {
        ...
        it("should announce a popover correctly by reading the aria described by label", async () => {
            // Arrange
            render(
                <Popover
                    onClose={jest.fn()}
                    aria-describedby="Popover Aria Described Label"
                    content={
                        <PopoverContentCore>
                            <button data-close-button onClick={close}>
                                Close Popover
            ...
            expect(popover).toHaveAttribute(
                "aria-describedby",
                "Popover Aria Described Label",
            );
            expect(popover).toHaveAttribute("aria-labelledby");
            expect(popover).not.toHaveAttribute("aria-label");
        });

        it("should announce a popover correctly by reading both the aria label and the aria described by label", async () => {
            // Arrange
            render(
                <Popover
                    onClose={jest.fn()}
