import {StyleSheet} from "aphrodite";
import {sizing} from "@khanacademy/wonder-blocks-tokens";

/**
 * A transparent hit area that guarantees a control is at least 24x24, to
 * satisfy WCAG 2.5.8 (Target Size, Minimum).
 *
 * We expand the *hit area* with a pseudo-element rather than growing the
 * element itself so that the visual layout of existing components is
 * unchanged.
 *
 * Notes:
 * - The hit area is centred on the control. With `inset: 0` the min-size would
 *   only grow down/right, off-centring the target.
 * - `blockSize`/`inlineSize` of `100%` mean the hit area is never *smaller*
 *   than the control it belongs to.
 * - There is deliberately no `pointerEvents` declaration. A pseudo-element is
 *   part of its host's hit area by default, so clicks on it dispatch to the
 *   host.
 * - WCAG 2.5.8 exempts inline targets (a link within a sentence, whose size is
 *   constrained by the line-height of the surrounding text), so this should not
 *   be applied to inline links.
 */
export const minTargetSizeStyles = StyleSheet.create({
    minTargetSize: {
        // Establishes the containing block for the ::after hit area below.
        position: "relative",
        // Aphrodite supports both `:after` and `::after` (its only check is
        // that the key starts with `:`, see `pseudoSelectors` in
        // aphrodite/src/generate.js). We use the double-colon form because it
        // is the correct modern syntax for a pseudo-element.
        "::after": {
            content: "''",
            position: "absolute",
            insetBlockStart: "50%",
            insetInlineStart: "50%",
            transform: "translate(-50%, -50%)",
            blockSize: "100%",
            inlineSize: "100%",
            minBlockSize: sizing.size_240,
            minInlineSize: sizing.size_240,
        },
    },
});
