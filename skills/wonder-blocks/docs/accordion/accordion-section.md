...
|------|------|---------|-------------|
| `id` | `string` |  | The unique identifier for the accordion section. |
| `children` | `string \| React.ReactElement` | *required* | The content to display when this section is shown. If a string is |
| `header` | `string \| React.ReactElement` | *required* | The header for this section. If a string is passed in, it will |
| `caretPosition` | `"start" \| "end"` |  | Whether to put the caret at the start or end of the header block |
| `cornerKind` | `AccordionCornerKindType` |  | The preset styles for the corners of this accordion. |
| `collapsible` | `boolean` |  | Whether this section is collapsible. If false, the header will not be |
| `expanded` | `boolean` |  | Whether this section is expanded or closed. |
| `animated` | `boolean` |  | Whether to include animation on the header. This should be false |
| `onToggle` | `(newExpandedState: boolean) => unknown` |  | Called when the header is clicked. |
| `style` | `StyleType` |  | Custom styles for the overall accordion section container. |
| `headerStyle` | `StyleType` |  | Custom styles for the header. |
| `tag` | `"h1" \| "h2" \| "h3" \| "h4" \| "h5" \| "h6"` |  | The semantic tag for this clickable header (e.g. "h1", "h2", etc). |
| `testId` | `string` |  | The test ID used to locate this component in automated tests. |
| `isFirstSection` | `boolean` |  | Whether this section is the first section in the accordion. |
| `isLastSection` | `boolean` |  | Whether this section is the last section in the accordion. |
| `isRegion` | `boolean` |  | Whether this section should have role="region". True by default. |
...
```tsx
<AccordionSection header="Standalone section" caretPosition="end" cornerKind="rounded" collapsible expanded={false}>
  {"This is the information present in this standalone section"}
</AccordionSection>
```
