...
This can be used after [`initializeHydrationCache`](./?path=/docs/packages-data-exports-initializehydrationcache--docs) to manipulate the cache prior to hydration wich can be useful during testing (especially to clear the cache so that it can be initialized again).
