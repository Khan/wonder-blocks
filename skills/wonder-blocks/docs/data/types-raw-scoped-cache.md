...
`RawScopedCache` describes a cache that has distinct scoped sections in its raw object form. This is the representation of the caches used internally by Wonder Blocks Data to support the scoping of requests when using hooks such as [`useSharedCache`](./?path=/docs/packages-data-exports-use-shared-cache--docs), [`useCachedEffect`](./?path=/docs/packages-data-exports-use-cached-effect--docs), and [`useHydratableEffect`](./?path=/docs/packages-data-exports-use-hydratable-effect--docs).

See the section on [server-side rendering](./?path=/docs/packages-data-server-side-rendering-and-hydration--docs) for more information.
