...
This type represents the mocking API for `fetch()`. It is the type of the function returned by the [`mockFetch()`](./?path=/docs/packages-testing-mocking-exports-mockfetch--docs), which is both a drop in replacement for a `fetch()`-like method, but also exposes the mocking API for configuration additional mocks.
