...
There are some [default adapters](./?path=/docs/packages-testing-test-harness-exports-harnessadapters--docs) provided as well as default [`testHarness()`](./?path=/docs/packages-testing-test-harness-exports-testharness--docs) and [`hookHarness()`](./?path=/docs/packages-testing-test-harness-exports-hookharness--docs) methods that use these adapters with meaningful defaults, which can be overridden.

However, it is likely you will want to replace or augment the existing adapters with some of your own. For this purpose, the [`makeTestHarness()`](./?path=/docs/packages-testing-test-harness-exports-maketestharness--docs) and [`makeHookHarness()`](./?path=/docs/packages-testing-test-harness-exports-makehookharness--docs) methods are provided.
